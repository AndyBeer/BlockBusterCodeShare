    ...
    internal class BlockbusterLog
    {
        ...
        public void movieSelection()
        {
            while (true)
            {
                try
                {
                    Console.Write("\nEnter a moive ID: "); //Spellcheck! XD
                    int movie = int.Parse(Console.ReadLine());  //Exception unhandled - used a really large # broke it
                    if (movie <= MovieTime.Count)
                    {
                        Movie m1 = MovieTime[movie]; //Exception unhandled - negative number broke it
                        ...
                        while (true)
                        {
                            ...
                            {
                                Console.Write("\nEnter a Scene ID: ");
                                int selection = Convert.ToInt32(Console.ReadLine()); //exception unhandled - really large # broke it
                            ...
                            catch (FormatException e) //didnt need the FormatException variable since we arent returning any messaging - not broken, but not needed.
                            {
                                Console.WriteLine("Please only enter a number from the scene list");
                                //you could add something like this below, and it will print out the exception message:
